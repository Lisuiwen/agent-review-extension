...
| 是否修改业务代码 / 产生 Git 脏区 | **仅注释** | 放行采用 **`@ai-ignore` 内联注释**，写入工作区代码上方一行，可追溯、可版本管理。 |
| 放行粒度 | **行级（永久）** | 通过“放行此条”在问题行上方插入 `@ai-ignore`，该行（及注释下一行）后续审查被过滤；无“单次提交放行”功能。 |

**实现位置摘要：**

- `allowIssueIgnoreCommand.ts`：在问题行上方插入 `// @ai-ignore: <原因>`（或对应语言注释格式）。
- `reviewEngine.filterIgnoredIssues`：审查结果合并后按文件中 `@ai-ignore` 覆盖行过滤 issue。
- `reviewPanel.syncAfterIssueIgnore`：插入注释后本地同步行号与“已放行”标记，刷新 TreeView。

### 结论与差距

- **已实现报告推荐的细粒度放行**：代码内联注释、与代码物理绑定、可追溯。
- **无“单次提交放行”**：仅支持永久放行（@ai-ignore），不支持整次 commit 临时跳过。
...
| 维度 | 现状 | 说明 |
|------|------|------|
| 是否自动探测项目规范 | **是** | 配置加载阶段会探测 .eslintrc、tsconfig.json、prettier 配置等文件。 |
| 是否“有项目规则则默认开启规则审查” | **是（自动默认）** | 当用户未显式设置 `builtin_rules_enabled` 且探测到项目规则文件时，自动将其设为 `true`。 |
| 侧边栏是否“增量 vs 存量”分栏 | **是** | `reviewPanel.ts` 根节点固定展示“你的增量 / 项目存量”两栏；增量默认展开、存量默认折叠，数量可为 0。 |

**实现位置摘要：**

- `configManager.ts`：`detectProjectRuleConfig` 会探测 ESLint/TS/Prettier 规则文件；若用户未显式配置 `builtin_rules_enabled`，探测到规则文件时默认开启。
- `types/config.ts`：`builtin_rules_enabled` 仍可由用户显式覆盖。
- `ui/reviewPanel.ts`：`getChildren` 根节点固定展示“你的增量 / 项目存量”分组；分组下再按文件聚合问题。

### 结论与差距

- **已实现关键能力**：支持探测项目规范文件并在未显式配置时自动开启规则审查；侧边栏已实现“你的增量 / 项目存量”分栏，且默认展开/折叠策略生效。
- **剩余差距**：  
  - 当前探测基于“规则文件存在性”，尚未深度解析外部规则内容并映射到内置规则行为。
...
|--------|--------------------------|----------|--------------------|
| 1. 本次放行机制 | **否**（已避免行号+脏区） | 仅支持“放行此条”（`@ai-ignore`，行级永久放行）；无单次/整次提交放行 | 中：若需“单次提交放行”需单独设计 |
| 2. AST 上下文致盲 | **是** | 无 LSP、无外部上下文、无 Prompt 隔离 | 高：直接影响 AI 误报率 |
| 3. Pre-commit 性能 | **否** | pre-commit 仅规则、不跑 AI；AI 仅命令触发 | 中：可做“保存时/无感”与漏斗式检测 |
| 4. 规范碎片化与分栏 | **部分** | 已实现规则文件探测（探测到项目规则时默认开启）与“增量/存量”分栏；未做规则内容深度映射 | 高：规则映射与体验细化 |
