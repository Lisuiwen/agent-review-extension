...
describe('ReviewEngine', () => {
    ...
    describe('测试用例 7.2: 非 Strict Mode', () => {
        ...
        it('非 strict_mode 下 block_commit error 应阻止提交', async () => {
            ...
            // block_commit 级别的错误应该阻止提交
            expect(result.errors.length).toBeGreaterThan(0);
            expect(result.passed).toBe(false);
        });

        it('非严格模式下 warning 级别的错误笉应该阻止提交', async () => {
    ...
    describe('测试用例 7.3: 规则 action 映射表', () => {
        it('应该使用映射表而非硬编码来检查 rule 的 action', async () => {
            const configManager = createMockConfigManager({
                rules: {
                    enabled: true,
                    strict_mode: false,
            ...
            // 应该正确识别 block_commit 与 warning 的区别
        ...
        it('no_debugger 为 block_commit 时应阻止提交', async () => {
            const configManager = createMockConfigManager({
                rules: {
                    enabled: true,
                    strict_mode: false,
